 ...
 */
// Last update: Dynamic FEC (Forward Error Correction)
...
// --- Livello interferenze ---
// coding rate supportati da LoRa
enum CodingRate {
    cr_low = 1, // 4/5 (veloce ma poco protetto)
    cr_mid = 2, // 4/6 
    cr_high = 3, // 4/7
    cr_max = 4, // 4/8 (molto portetto ma lento)
};

// calcolo delle interferenze
int calculateInterference(uint8_t livello_interferenze) { // 8 byte per non occupare spazio extra per arrivare al numero 10 (1010)
    int level = foreign_packets_count / 10; // ogni 10 pacchetti estranei, aumentiamo la variabile
...
CodingRate DynamicFEC(uint8_t livello_interferenze) {
    if (interference_level <= 2) {
        return cr_low;  
    } else if (interference_level <= 5) {
        return cr_mid; 
    } else if (interference_level <= 8) {
        return cr_high; 
    } else {
        return cr_max; 
    }
